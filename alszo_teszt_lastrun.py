...
filename = _thisDir + os.sep + u'data/%s_%s_%s' % (expInfo['Résztvevő kódja'], expName, expInfo['date'])
...
inst_text = visual.TextStim(win=win, name='inst_text',
    text='Kedves résztvevő!\n\nA következő feladatban különböző hosszúságú, furcsa hangzású, értelmetlen szavakat fog meghallgatni. Próbálja meg a lehető legpontosabban visszamondani őket a kísérletvezetőnek!\n\nA soron következő szót a kísérletvezető fogja elindítani.\n\nFontos, hogy végig nagyon figyeljen, mert egy szót csak egyszer fog hallani!',
    font='Open Sans',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0.0, 
    color='black', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=0.0);
space_inst = visual.TextStim(win=win, name='space_inst',
    text='A kezdéshez nyomja meg a SPACE gombot!',
    font='Open Sans',
...
text = visual.TextStim(win=win, name='text',
    text='Készüljön, a kísérlet hamarosan elindul!',
    font='Open Sans',
...
text_2 = visual.TextStim(win=win, name='text_2',
    text='Ismételje el a hallott szót!',
    font='Open Sans',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0.0, 
    color='black', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
# Initialize components for Routine "vege"
vegeClock = core.Clock()
kosznojuk = visual.TextStim(win=win, name='kosznojuk',
    text='Köszönjük a részvételt!\n\nA SPACE gomb megnyomásával kiléphet a kísérletből.',
    font='Open Sans',
    pos=(0, 0), height=0.07, wrapWidth=None, ori=0.0, 
    color='black', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
instrComponents = [inst_text, space_inst, key_resp]
...
while continueRoutine:
    ...
    # *space_inst* updates
    if space_inst.status == NOT_STARTED and tThisFlip >= 8.0-frameTolerance:
        # keep track of start time/frame for later
        space_inst.frameNStart = frameN  # exact frame index
        space_inst.tStart = t  # local t and not account for scr refresh
        space_inst.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(space_inst, 'tStartRefresh')  # time at next scr refresh
        space_inst.setAutoDraw(True)
    
    # *key_resp* updates
    waitOnFlip = False
    if key_resp.status == NOT_STARTED and tThisFlip >= 8.0-frameTolerance:
        # keep track of start time/frame for later
...
thisExp.addData('space_inst.started', space_inst.tStartRefresh)
thisExp.addData('space_inst.stopped', space_inst.tStopRefresh)
# check responses
...
# abbreviate parameter names if possible (e.g. rgb = thisHangok.rgb)
if thisHangok != None:
    for paramName in thisHangok:
        exec('{} = thisHangok[paramName]'.format(paramName))

for thisHangok in hangok:
    currentLoop = hangok
    # abbreviate parameter names if possible (e.g. rgb = thisHangok.rgb)
    if thisHangok != None:
        for paramName in thisHangok:
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    fxClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        # get current time
        t = fxClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=fxClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    for thisComponent in fxComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    hangok.addData('fx_cross.started', fx_cross.tStartRefresh)
    hangok.addData('fx_cross.stopped', fx_cross.tStopRefresh)
    ...
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    trialComponents = [sound_1, key_resp_2, text_2]
    ...
    while continueRoutine:
        ...
        if key_resp_2.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_2.getKeys(keyList=['u'], waitRelease=False)
    ...
    hangok.addData('sound_1.started', sound_1.tStartRefresh)
    hangok.addData('sound_1.stopped', sound_1.tStopRefresh)
    # check responses
    if key_resp_2.keys in ['', [], None]:  # No response was made
        key_resp_2.keys = None
    hangok.addData('key_resp_2.keys',key_resp_2.keys)
    if key_resp_2.keys != None:  # we had a response
        hangok.addData('key_resp_2.rt', key_resp_2.rt)
    hangok.addData('key_resp_2.started', key_resp_2.tStartRefresh)
    hangok.addData('key_resp_2.stopped', key_resp_2.tStopRefresh)
    hangok.addData('text_2.started', text_2.tStartRefresh)
    hangok.addData('text_2.stopped', text_2.tStopRefresh)
    # the Routine "trial" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 1.0 repeats of 'hangok'
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
vegeClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1
...
while continueRoutine:
    # get current time
    t = vegeClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=vegeClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *kosznojuk* updates
    if kosznojuk.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        kosznojuk.frameNStart = frameN  # exact frame index
        kosznojuk.tStart = t  # local t and not account for scr refresh
        kosznojuk.tStartRefresh = tThisFlipGlobal  # on global time
